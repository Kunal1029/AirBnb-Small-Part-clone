  ...
  wrapAsync(async (req, res) => {
    if (!req.body.listing) {
      throw new ExpressError(
        404,
        "Send valid data because you are sending empty data or invalid data"
      );
    }
    let { id } = req.params;
...
app.use((err, req, res, next) => {
  let {
    statusCode = 500,
    message = "This error message will reflect when error occurs but express error message not given otherwise express error message will display",
  } = err;
